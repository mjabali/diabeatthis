        ...
        <v-card v-if="!credentialsProvided">
          ...
          <v-card-text>
            ...
            <v-form ref="loginForm" v-model="valid">
              ...
              ></v-text-field>
              <v-select
                v-model="dexcomServer"
                :items="['US', 'EU']"
                label="Dexcom Server location"
                required
              ></v-select>
            </v-form>
          </v-card-text>
          <v-card-actions>
            <v-btn @click="handleLogin" :disabled="!valid" block> Login </v-btn>
          </v-card-actions>
        </v-card>

        <!-- LOADING -->
        <div
          v-if="(isLoggedIn || credentialsProvided) && !dataRetrieved"
          class="text-center"
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      var seconds = moment().diff(
        moment(this.history[this.history.length - 1].time),
        "seconds"
      );
      if (seconds > 60 * 5) {
        console.log(" - Fetching new data...");
        this.dexcomClient.fetchReadings(60 * 24, 12 * 24).then((data) => {
