...
export default {
  ...
  methods: {
    async updateChartData(readings) {
      ...
      var smoothedReadings = smoothData(
        readings.map((reading) => reading.mmol)
      ).slice(-this.amountOfDataPoints);
      ...
      const colorsActualReadings = newReadings.map((reading) =>
        getColorForReading(
          reading.mmol,
          minValue,
          maxValue,
      ...
      const colorsSmoothedReadings = smoothedReadings.map((reading) =>
        getColorForReading(
          reading,
          minValue,
          maxValue,
      ...
      var alpha = 0;
      var ses = new zodiac.DoubleExponentialSmoothing(
        readings.map((reading) => reading.mmol),
        alpha
      );
