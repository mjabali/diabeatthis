<template>
  <span v-if="valueOnly">
    <span :class="inRange >= 70 ? 'readingIn' : 'readingOut'"
      >TIR: {{ inRange }}%</span
    >&nbsp;
    <span
      :class="
        history[history.length - 1].mmol >= 4 &&
        history[history.length - 1].mmol <= 10
          ? 'readingIn'
          : 'readingOut'
      "
      >&#11044;</span
  ...
  <div v-else class="text-center">
    ...
    >
      <template v-slot:default
        ><span class="value">{{ inRange }}%</span></template
      >
    </v-progress-circular>
  </div>
</template>
  ...
  methods: {
    updateInRange(history) {
      ...
      const inRange = history.filter(
        (reading) => reading.mmol >= 80 && reading.mmol <= 180
      ).length;
      this.inRange = Math.round((inRange / history.length) * 100);

      const now = new Date();
      const readingsInDay = (24 * 60) / 5;
      const missingReadings = Math.round(
        (24 * 60 - (now.getHours() * 60 + now.getMinutes())) / 5
      );
      this.min = Math.round((inRange / readingsInDay) * 100);
      this.max = Math.round(
        ((inRange + missingReadings) / readingsInDay) * 100
      );
    },
  },
  mounted() {
